r"""Batch four-way NMR comparison workflow for RTSPHEM experiment folders.

Usage examples, from the ``conda ml`` environment:

    python C:\Users\imgw\Documents\Codex\NMR模拟\advanced_tools\run_four_way_nmr_comparison_gpu.py ^
...
Compared with run_three_way_nmr_comparison.py, this script adds a fourth method:
PNG pixel GPU. The GPU branch uses the same phase map, same finite-difference
operator, same surface relaxation terms, and same implicit Euler equation as
the CPU pixel branch, but solves the sparse linear system on CUDA through CuPy.
"""
...
# 7. GPU pixel 求解参数
# 这是新增方法的开关。True 时会多跑一条 PNG pixel GPU 曲线；
# 如果机器没有 CUDA/CuPy，脚本会明确报错，而不是静默伪装成 GPU 结果。
ENABLE_PIXEL_GPU = True

# GPU 稀疏求解默认使用共轭梯度 cg。这个隐式 Euler 矩阵是对称正定矩阵，
# 适合 CG；比 CuPy 的 sparse factorized 路径更稳。factorized/spsolve 在
# 某些 CUDA/cuSPARSE 组合上可能触发 illegal memory access。
PIXEL_GPU_SOLVER = "cg"  # "cg", "factorized", or "spsolve"

# CG 收敛参数。容差越小越接近 CPU sparse LU 结果，但耗时越长。
...
try:
    ...
    import cupyx.scipy.sparse as cpsparse  # noqa: E402
    import cupyx.scipy.sparse.linalg as cplinalg  # noqa: E402
...
def info(message: str) -> None:
    print(f"[four-way-nmr] {message}", flush=True)
...
def solve_decay_pixel_gpu(labels: np.ndarray, params: SimulationParams) -> tuple[np.ndarray, np.ndarray, dict[str, float]]:
    """GPU version of the existing pixel implicit-Euler solver.

    The expensive numerical solve is moved to CUDA, but the operator assembly is
    intentionally reused from build_water_operator(). That keeps water-solid and
    water-gas boundary interpretation exactly aligned with the CPU pixel solver.
    """

    if not CUPY_AVAILABLE:
        raise RuntimeError("CuPy is not available in this Python environment.")
    device_count = int(cp.cuda.runtime.getDeviceCount())
    if device_count < 1:
        raise RuntimeError("CuPy is installed, but no CUDA device is available.")

    lap_cpu, solid_sink_cpu, gas_sink_cpu, boundary_counts = build_water_operator(labels, params)
    n = lap_cpu.shape[0]
    dt = params.dt_ms

    lap_gpu = cpsparse.csr_matrix(lap_cpu)
    lhs_gpu = (
        cpsparse.eye(n, format="csr", dtype=cp.float64)
        + lap_gpu * (params.diffusion_um2_per_ms * dt)
        + cpsparse.eye(n, format="csr", dtype=cp.float64) * (dt / params.bulk_t2_ms)
        + cpsparse.diags(
            cp.asarray(params.rho_solid_um_per_ms * dt * solid_sink_cpu, dtype=cp.float64),
            format="csr",
        )
        + cpsparse.diags(
            cp.asarray(params.rho_gas_um_per_ms * dt * gas_sink_cpu, dtype=cp.float64),
            format="csr",
        )
    )

    if PIXEL_GPU_SOLVER == "factorized":
        solve = cplinalg.factorized(lhs_gpu.tocsc())
    elif PIXEL_GPU_SOLVER == "spsolve":
        solve = lambda rhs: cplinalg.spsolve(lhs_gpu.tocsr(), rhs)
    elif PIXEL_GPU_SOLVER == "cg":
        lhs_csr_gpu = lhs_gpu.tocsr()

        def solve(rhs):
            x, cg_info = cplinalg.cg(
                lhs_csr_gpu,
                rhs,
                x0=rhs,
                tol=PIXEL_GPU_CG_TOL,
                maxiter=PIXEL_GPU_CG_MAXITER,
            )
            if int(cg_info) != 0:
                raise RuntimeError(f"CuPy CG did not converge; info={int(cg_info)}")
            return x

    else:
        raise ValueError(f"Unsupported PIXEL_GPU_SOLVER: {PIXEL_GPU_SOLVER}")

    times = np.arange(0.0, params.t_max_ms + 0.5 * dt, dt)
    magnetization = cp.ones(n, dtype=cp.float64)
    amplitude_gpu = cp.empty(times.size, dtype=cp.float64)
    pixel_area_um2 = params.pixel_size_x_um * params.pixel_size_y_um
    for i in range(times.size):
        amplitude_gpu[i] = cp.sum(magnetization) * pixel_area_um2
    ...
    boundary_counts.update(
        {
            "backend": "cupy",
            "gpu_solver": PIXEL_GPU_SOLVER,
            "gpu_cg_tol": PIXEL_GPU_CG_TOL if PIXEL_GPU_SOLVER == "cg" else None,
            "gpu_cg_maxiter": PIXEL_GPU_CG_MAXITER if PIXEL_GPU_SOLVER == "cg" else None,
            "cuda_device_count": device_count,
            "cuda_device_name": cp.cuda.runtime.getDeviceProperties(0)["name"].decode("utf-8", errors="replace"),
        }
    )
    return times, amplitude, boundary_counts


def simulate_png_gpu_pixel(png_path: Path, output_dir: Path, params: SimulationParams) -> dict[str, object]:
    """Run the PNG pixel solver with CUDA and write the same decay CSV format."""
    ...
    time_ms, amplitude, boundary_counts = solve_decay_pixel_gpu(labels, effective_params)
    ...
    save_decay_plot(time_ms, normalized, plot_path, title=f"{stem} T2 decay (PNG pixel GPU)")
...
def run_pixel_gpu_decay(steps: list[dict], output_dir: Path, params: SimulationParams) -> dict[int, float]:
    if not ENABLE_PIXEL_GPU:
        return {}
    output_dir.mkdir(parents=True, exist_ok=True)
    summaries = []
    timings: dict[int, float] = {}
    for item in steps:
        step = item["timestep"]
        info(f"pixel GPU decay timestep {step:04d}")
        start = time.perf_counter()
        summaries.append(simulate_png_gpu_pixel(item["png"], output_dir, params))
        timings[step] = time.perf_counter() - start
    (output_dir / "png_phase_nmr_decay_summary.json").write_text(
...
def make_four_way_frames(
    steps: list[dict],
    exp_dir: Path,
    output_dir: Path,
    pixel_decay_dir: Path,
    pixel_gpu_decay_dir: Path,
    ...
    for frame_idx, item in enumerate(steps, start=1):
        step = item["timestep"]
        info(f"plot four-way comparison timestep {step:04d}")
        pixel_csv = pixel_decay_dir / f"timestep_{step:04d}_nmr_decay.csv"
        pixel_gpu_csv = pixel_gpu_decay_dir / f"timestep_{step:04d}_nmr_decay.csv"
        ...
        pixel_gpu_t_raw, pixel_gpu_s_raw = read_png_decay_csv(pixel_gpu_csv)
        ...
        pixel_gpu_inv = invert_fixed_alpha(
            pixel_gpu_t,
            pixel_gpu_s,
            name=f"PNG_pixel_GPU_t{step:04d}",
            alpha=NNLS_ALPHA,
            t2_min_ms=T2_MIN_MS,
            t2_max_ms=T2_MAX_MS,
            num_bins=T2_NUM_BINS,
            ...
            pd.DataFrame(
                {
                    ...
                    "png_pixel_gpu_spectrum_norm": pixel_gpu_spec,
                    "png_triangular_mesh_spectrum_norm": tri_spec,
                }
            ).to_csv(output_dir / f"timestep_{step:04d}_t2_spectrum_four_way.csv", index=False)
            pd.DataFrame(
                {
                    ...
                    "png_pixel_gpu_time_ms": pd.Series(pixel_gpu_t),
                    "png_pixel_gpu_signal_norm": pd.Series(normalize(pixel_gpu_s)),
                    "png_triangular_time_ms": pd.Series(tri_t),
                    "png_triangular_signal_norm": pd.Series(normalize(tri_s)),
                }
            ).to_csv(output_dir / f"timestep_{step:04d}_decay_four_way.csv", index=False)
        ...
        ax_decay.plot(pixel_gpu_t, normalize(pixel_gpu_s), color="#9467bd", lw=1.7, ls=":", label="PNG pixel GPU")
        ...
        ax_t2.plot(pixel_gpu_inv.t2_bins_ms, pixel_gpu_spec, color="#9467bd", lw=1.7, ls=":", label="PNG pixel GPU")
        ...
        if frame_idx == 1:
            first_static_png = output_dir / f"timestep_{step:04d}_four_way_decay_t2_comparison.png"
        ...
        summary_rows.append(
            {
                ...
                "png_pixel_gpu_peak_t2_ms": float(pixel_gpu_inv.t2_bins_ms[int(np.argmax(pixel_gpu_inv.spectrum))]),
                ...
                "png_pixel_gpu_peak_time_raw_ms": float(pixel_gpu_t_raw[pixel_gpu_peak]),
                "png_triangular_peak_time_raw_ms": float(tri_t_raw[tri_peak]),
                "pixel_cpu_gpu_max_abs_signal_diff": float(
                    np.max(np.abs(normalize(pixel_s) - normalize(pixel_gpu_s)))
                    if pixel_s.shape == pixel_gpu_s.shape
                    else math.nan
                ),
            }
        )
...
def make_timing_outputs(
    ...
    pixel_gpu_timing: dict[int, float],
        ...
        rows.append(
            {
                ...
                "png_pixel_gpu_wall_time_s": pixel_gpu_timing.get(step, math.nan),
    ...
    series = [
        ...
        ("png_pixel_gpu_wall_time_s", "PNG pixel GPU wall time", "D", "#9467bd"),
    ...
    box_data = [
        ...
        frame["png_pixel_gpu_wall_time_s"].dropna().to_numpy(),
        frame["png_triangular_wall_time_s"].dropna().to_numpy(),
    ]
    labels = ["COMSOL", "PNG pixel", "PNG pixel GPU", "PNG mesh"]
    try:
        ax_box.boxplot(box_data, tick_labels=labels, showmeans=True)
    except TypeError:
        ax_box.boxplot(box_data, labels=labels, showmeans=True)
...
def main() -> None:
    ...
    output_dir = exp_dir / f"four_way_nmr_gpu_workflow_{run_id}"
    ...
    pixel_gpu_decay_dir = output_dir / "png_pixel_gpu_decay"
    ...
    pixel_gpu_params = build_params(pixel_size_x_um, pixel_size_y_um, solver="pixel_gpu")
    ...
    pixel_gpu_timing = run_pixel_gpu_decay(steps, pixel_gpu_decay_dir, pixel_gpu_params)
    ...
    summary_rows, first_static_png = make_four_way_frames(
        steps,
        exp_dir,
        output_dir,
        pixel_decay_dir,
    ...
    summary_csv = output_dir / "four_way_decay_t2_summary.csv"
    pd.DataFrame(summary_rows).to_csv(summary_csv, index=False)

    video_path = output_dir / f"{exp_dir.name}_four_way_decay_t2_with_concentration_{alpha_token(NNLS_ALPHA)}.mp4"
    ...
    manifest = {
        ...
        "params": {
            ...
            "pixel_gpu": asdict(pixel_gpu_params),
            ...
            "pixel_gpu_solver": PIXEL_GPU_SOLVER,
        ...
        "outputs": {
            ...
            "png_pixel_gpu_decay_dir": str(pixel_gpu_decay_dir),
